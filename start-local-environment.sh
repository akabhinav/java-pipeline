...
start_services() {
    ...
    echo -e "  ${YELLOW}Spark Master:${NC}   http://localhost:8081  (Cluster UI)"
    echo -e "  ${YELLOW}Spark Worker 1:${NC} http://localhost:8082"
    echo -e "  ${YELLOW}Spark Worker 2:${NC} http://localhost:8083"
    echo -e "  ${YELLOW}Spark History:${NC}  http://localhost:18080"
    echo ""
    ...
    echo "  4. Run on Spark cluster:  ./run-pipeline-on-cluster.sh RealConnectorsPipeline"
    echo "  5. Stop services:         ./start-local-environment.sh --stop"
    echo ""
