...
/**
 * Add rank within partitions (leaves gaps in ranking for ties).
 *
 * Config:
 * - partitionBy: List of columns to partition by (optional)
 * - orderBy: List of columns to order by (required)
 * - alias: Column name for rank (default: "rank")
 *
 * Example:
 * {
 *   "type": "rank",
 ...
 * }
 *
 * Result: 1, 2, 2, 4, 5 (note gap after tie)
 *
 * @author Enterprise Data Pipeline Team
 */
public class RankTransform implements Transformation {

    private static final Logger logger = LoggerFactory.getLogger(RankTransform.class);
    ...
    public Dataset<Row> transform(Dataset<Row> input, Map<String, Object> config, TransformationContext context)
        ...
        try {
            ...
            String alias = config.containsKey("alias") ? (String) config.get("alias") : "rank";

            logger.debug("Adding rank with orderBy: {}, alias: {}", orderBy, alias);
            ...
            return input.withColumn(alias, functions.rank().over(windowSpec));

        } catch (Exception e) {
            throw new TransformationException(getName(),
                    "Failed to add rank: " + e.getMessage(), e);
        }
    }
}
