 ...
 */
public class LeftJoinTransform implements Transformation {

    private static final Logger logger = LoggerFactory.getLogger(LeftJoinTransform.class);
    ...
    public Dataset<Row> transform(Dataset<Row> input, Map<String, Object> config, TransformationContext context)
        ...
        try {
            ...
            logger.debug("Left joining with '{}' on columns: {}", rightDatasetName, joinColumns);
            ...
            throw new TransformationException(getName(),
                    "Failed to perform left join: " + e.getMessage(), e);
        }
    }
}
