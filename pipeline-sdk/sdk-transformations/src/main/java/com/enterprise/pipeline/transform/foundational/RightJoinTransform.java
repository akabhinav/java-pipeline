 ...
 */
public class RightJoinTransform implements Transformation {

    private static final Logger logger = LoggerFactory.getLogger(RightJoinTransform.class);
    ...
    public Dataset<Row> transform(Dataset<Row> input, Map<String, Object> config, TransformationContext context)
        ...
        try {
            ...
            logger.debug("Right joining with '{}' on columns: {}", rightDatasetName, joinColumns);
            ...
            throw new TransformationException(getName(),
                    "Failed to perform right join: " + e.getMessage(), e);
        }
    }
}
