...
/**
 * Left semi join with another dataset.
 * Returns only rows from left dataset that have matches in right dataset.
 *
 * Config:
 ...
 */
public class LeftSemiJoinTransform implements Transformation {

    private static final Logger logger = LoggerFactory.getLogger(LeftSemiJoinTransform.class);
    ...
    public Dataset<Row> transform(Dataset<Row> input, Map<String, Object> config, TransformationContext context)
        ...
        try {
            ...
            logger.debug("Left semi joining with '{}' on columns: {}", rightDatasetName, joinColumns);
            ...
            throw new TransformationException(getName(),
                    "Failed to perform left semi join: " + e.getMessage(), e);
        }
    }
}
