 ...
 *   "type": "fullOuterJoin",
 *   "config": {
 *     "rightDataset": "transactions",
 *     "joinColumns": ["transaction_id"]
 *   }
 * }
 *
 * @author Enterprise Data Pipeline Team
 */
public class FullOuterJoinTransform implements Transformation {

    private static final Logger logger = LoggerFactory.getLogger(FullOuterJoinTransform.class);
    ...
    public Dataset<Row> transform(Dataset<Row> input, Map<String, Object> config, TransformationContext context)
        ...
        try {
            ...
            logger.debug("Full outer joining with '{}' on columns: {}", rightDatasetName, joinColumns);
            ...
            throw new TransformationException(getName(),
                    "Failed to perform full outer join: " + e.getMessage(), e);
        }
    }
}
